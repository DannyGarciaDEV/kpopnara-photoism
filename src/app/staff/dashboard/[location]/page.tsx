...
export default function StaffDashboardLocation() {
  ...
  const slug = typeof params?.location === 'string' ? params.location.toLowerCase() : '';
  const locationIdFromSlug = SLUG_TO_LOCATION_ID[slug];
  ...
  useEffect(() => {
    if (!slug || !locationIdFromSlug) {
      router.replace('/staff/login');
      return;
    }
  }, [slug, locationIdFromSlug, router]);
  ...
  const fetchData = async () => {
    ...
    if (res.ok) {
      ...
      if (apiLocationId === 'nyc' && slug !== 'nyc') {
        router.replace('/staff/dashboard/nyc');
        return;
      }
